import { ArrayKeyedMap, ArraySet } from "./arrayKeyedMap";
import { dfsFrom } from "./dfs";
// import { getIntKey } from "./grid";
...
describe("DFS", () => {
  ...
  it("objects", () => {
    const stateMap = ArraySet([[0]]);

    const dfs = dfsFrom(
      stateMap.get([0]),
      n => {
        const [level, ...coords] = n;
        ...
        for (let i = 0; i < 3; i++) {
          coords[level] = i;
          const nextState = [level + 1, ...coords];

          next.push(stateMap.store(nextState));
        }
